...
qa/format:  ## Run code formatter
	@uv run ruff check --select I --fix ${PACKAGE_TARGET} tests src/chill_out_demo examples
	@uv run ruff format ${PACKAGE_TARGET} tests src/chill_out_demo examples
...
app: app/run  ## Shortcut for app/run

app/run:  ## Run the CLI application
	@uv run chill-out

app/debug:  ## Run the CLI application in debug mode
	@uv run debugpy --listen localhost:5678 --wait-for-client -m chill_out.cli.main
