...
A self-contained Python project that chill-out can audit. The fixture is
constructed so a deep check finds two violations:

- `httpx==0.27.0` is a fresh direct dependency inside the cooldown window;
  chill-out proposes pinning it back to `0.26.0`.
- `anyio==4.3.0` is a fresh transitive that fastapi pulls in. The installed
